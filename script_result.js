document.addEventListener('DOMContentLoaded', () => {
    ...
    // ランクに応じてOGP画像を切り替える
    if (point >= 9) {
        ogImageUrl = baseUrl + 'images/ogp_s.png'; // Sランク用
    } else if (point >= 6) {
        ogImageUrl = baseUrl + 'images/ogp_a.png'; // Aランク用
    } else if (point >= 3) {
        ogImageUrl = baseUrl + 'images/ogp_b.png'; // Bランク用
    } else {
        ogImageUrl = baseUrl + 'images/ogp_c.png'; // Cランク以下用
    }
    ...
    if (shareButton) {
        ...
        // 選択された都道府県の日本語名を取得（取得できない場合は、URLパラメータの値をそのまま利用）
        const displayPrefName = prefNameMap[selectedPref] || selectedPref;

        // シェア文言を構築
        const shareText = `【山形ツッコミオーディション】\n${displayPrefName}と山形県のコンビ相性は…\nランク：${simpleRank}\n称号：${typeName.replace(/\n/g, "")}\n\n`;
        
        // シェアURL（GitHub PagesのトップURL）
        const shareUrl = "https://hisa0u0.github.io/yamagata_tsukkomi_audition/";
        const hashtags = "山形ツッコミオーディション";

        // Twitter(X)のインテントURLをセット
